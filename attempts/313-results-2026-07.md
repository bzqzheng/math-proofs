...
**Reading.** Across $k=3,\dots,9$, $\Sigma_k$ stays in $[0.5,0.9]$ with no geometric decay:
$0.72,\,0.87,\,0.86,\,0.78,\,0.78,\,0.69,\,\ge0.51$. Extrapolating flat gives $\sum_k S_k=\infty$ —
**the heuristic points to YES for Erdős #313** — with the count of PPNs having exactly $k$ prime
factors staying $O(1)$ on average, i.e. the PPN counting function growing like $c\cdot k$ in the
number of prime factors (so like $\log\log\log n$ in $n$, since PPNs with $k$ factors are
doubly-exponentially large in $k$).

Honest caveat: seven data points cannot exclude a slow decline. A geometric fit on $k=4..9$ gives
ratio $\approx0.90$ and a finite total of $\approx5$ further PPNs. What tilts the reading toward YES
is the *composition*: the decaying part is the inheritance mass, and the growing part is the diffuse
mass, which is where all the room for growth is. The decisive quantity is the per-layer reproduction
number
$$\rho(N,A)=A\log\tfrac NA \sum_{\substack{q\ \mathrm{prime}\\ \max(m,N/A)<q<2N/A}}\frac1{(Aq-N)\log\frac{Nq}{Aq-N}},$$
averaged over the layer; $\rho>1$ means infinitude. Proving anything about $\overline\rho$ is the
natural next target and looks much more tractable than the original question.
...
1. **The direction now has quantitative support.** $\Sigma_k$ is a parameter-free prediction of
   $S_k$, it matches the exhaustive data for $k=3..8$, it ranks the correct prefix first at both
   $k=8$ and $k=9$ out of sample, and it stays in $[0.5,0.9]$ through $k=9$ with the composition
   shifting from inheritance to diffuse mass. Read flat, it says **yes, infinitely many**, with
   $\#\{\text{PPN}\le x\}\asymp\log\log\log x$.
2. **The decisive quantity is now explicit**: the per-layer reproduction number $\overline\rho$.
   $\overline\rho>1$ ⟺ infinitude, and it is a statement about a concrete prime-density sum, not
   about PPNs.
