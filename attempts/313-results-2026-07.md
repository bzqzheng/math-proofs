...
Length 5 currently reproduces the inherited filling
$157\cdot1979\cdot10093\cdot16879\cdot5998279018951962403$ (→ $N_{10}$); the length-5 sweep is
still running.
...
For a PPN $n$ (port $(n,1)$), a 3-prime successor means primes $n<q_1<q_2<q_3$ with, writing
$u_i=q_i-n$,
$$u_1u_2u_3-n^2(u_1+u_2+u_3)=1+2n^3,\qquad\text{i.e.}\qquad
u_3=\frac{1+2n^3+n^2(u_1+u_2)}{u_1u_2-n^2},$$
so $u_1<2n$ and $u_1u_2$ must exceed $n^2$ by a divisor of the numerator. Every route needs either
$\sim2\times10^{8}$ factorisations of 38-digit numbers (one per $q_1$) or a 2-dimensional sweep of
size $\sim10^{18}$. **3-prime successors of a PPN are out of reach of current methods**, and they are
exactly what stands between today's technology and a complete $r=9$ theorem. Nodes with $t\ge3$ and
$A$ *not* small are fine; the obstruction is specifically the $A=1$ (i.e. PPN-prefix) nodes.
