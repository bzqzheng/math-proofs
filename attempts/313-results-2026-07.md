...
**Length 5 is also complete**: 5,721,645 terminal states, one filling —
$157\cdot1979\cdot10093\cdot16879\cdot5998279018951962403$, the inherited one giving $N_{10}$ —
plus 17,245 $t=2$ nodes settled by factoring, and exactly **two** $t\ge3$ nodes over budget:

* $P=2,3,11,17,101,151,2437$ ($A=8303$), loop width $1.0\times10^{7}$ — rerun at a higher budget;
...
The depth-$(k-1)$ level is far too large to enumerate, so the search is stopped at depth $k-2$
and the children are integrated analytically (substituting $x=Aq-N$, $dx/x=dy$ makes the integrand
smooth). Computed values (`attempts/ppn_heuristic.py`, and `HMODE=1 ./ppn k`):

| $k$ | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|
| $\Sigma_k$ | 0.716 | 0.874 | 0.855 | 0.779 | 0.781 | 0.689 | $\ge0.508$ |
| … from near-PPN states ($A\le100$) | 0.716 | 0.874 | 0.855 | 0.770 | 0.462 | 0.202 | — |
| … diffuse remainder | 0.000 | 0.000 | 0.000 | 0.009 | 0.319 | 0.487 | — |
| largest single state | 0.716 | 0.822 | 0.580 | 0.320 | 0.226 | 0.114 | 0.100 |
| $S_k$ (exhaustive) | 1 | 1 | 1 | 1 | 1 | 1 | $\ge1$ |

The decomposition is the interesting part. Small-$A$ states are the near-PPN ones — $A=1$ means the
prefix *is* a PPN, so the mass there is the classical inheritance mechanism. That mass **decays
fast** (0.87 → 0.20). What keeps $\Sigma_k$ near $0.7$ is the **diffuse remainder**, which **grows**
(0.000 → 0.487): the tree is expanding fast enough to replace the inheritance mass with a sea of
individually negligible states. The largest single-state contribution falls from 0.72 to 0.10, so
the estimate is becoming less lumpy — less variance, more trustworthy — exactly as $k$ grows.
...
**Reading — and it is the opposite of the naive one.** The sequence
$0.72,\,0.87,\,0.86,\,0.78,\,0.78,\,0.69,\,\ge0.51$ looks flat, but its ratios are
$1.22,\,0.98,\,0.91,\,1.00,\,0.88,\,0.74$ — settling toward $\log2$. If the limit really is
$\log2<1$ then
$$\sum_k S_k \;\approx\; \sum_k \Sigma_k \;<\;\infty,$$
i.e. **the heuristic predicts that there are only finitely many primary pseudoperfect numbers, and
the answer to Erdős #313 is NO.** Quantitatively: $\sum_{k\le9}\Sigma_k\approx5.2$ against 9 actual
PPNs with $k\le9$, so the model runs low by a factor $\approx1.7$; correcting for that,
$\sum_{k\ge10}\Sigma_k\approx1.7\cdot0.51\cdot\frac{0.69}{1-0.69}\approx1.9$. One of those is already
in hand ($N_{10}$). **The prediction is therefore: about one further primary pseudoperfect number
exists, and the sequence A054377 is finite with roughly 11 terms.**

Caveats, stated plainly: this is a heuristic, it assumes independence of the divisibility and
primality events, the $\rho\to\log2$ derivation replaces a prime sum by an integral, and seven data
points cannot by themselves distinguish ratio $0.87$ from ratio $1.00$. What makes it worth taking
seriously is that the constant is *derived*, not fitted, and that it agrees with two independent
measurements (the per-depth ratios and the $\Sigma_k$ ratios).
...
(`attempts/run_k9.sh`, resumable via `k9/*.done`).
...
1. **The direction now has quantitative support, and it points to NO.** $\Sigma_k$ is a
   parameter-free prediction of $S_k$; it matches the exhaustive data for $k=3..8$ and ranks the
   correct prefix first at both $k=8$ and $k=9$ out of sample. Its layer-to-layer ratio is
   governed by a *derived* constant, $\log 2$, giving a convergent total: **finitely many primary
   pseudoperfect numbers, about one more than the ten now known.** This is a falsifiable prediction:
   finding two or more further PPNs would refute the model.
2. **The decisive quantity is now explicit**: the per-layer reproduction number $\rho$, and it has a
   closed form, $\rho\approx\log\frac{2\log R-\log d_0}{\log R-\log(t-1)}$ with $R=N/A$. It is a
   statement about a concrete prime-density sum, not about PPNs, so it is the natural place to try
   for a theorem.
