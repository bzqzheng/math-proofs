...
static long mono_total;     /* current #mono APs */
...
static void apply_move(int i, int b) {
    ...
    for (int p = inc_off[i]; p < inc_off[i + 1]; p++) {
        ...
        if (cj[a] == K) mono_total--;      /* j stops being a-mono */
        cj[a]--;
        cj[b]++;
        if (cj[b] == K) mono_total++;      /* j becomes b-mono */
    }
    col[i] = b;
}

static int step(long st) {
    if (mono_total == 0) return 0;
    ...
    if (j < 0) { /* mono APs too rare for rejection: scan and collect one */
        long seen = 0, pick = rng_below(mono_total);
    ...
    if (rng_below(1000) < (unsigned)(NOISE * 1000)) {
        ...
        for (int t = 0; t < K; t++) {
            ...
            if (tabu[i] > st && mono_total > 1) { for (int c = 0; c < R; c++) w[t][c] = 0; continue; }
            int make = 0;
            for (int p = inc_off[i]; p < inc_off[i + 1]; p++) {
                unsigned char *cj = &cnt[(size_t)inc_list[p] * R];
                if (cj[a] == K) make++;      /* mono APs destroyed by flipping i */
            }
            for (int c = 0; c < R; c++) {
                ...
                for (int p = inc_off[i]; p < inc_off[i + 1]; p++) {
                    unsigned char *cj = &cnt[(size_t)inc_list[p] * R];
                    if (cj[c] == K - 1) brk++;  /* APs that would become c-mono */
                }
                int net = brk - make;           /* net new mono APs */
                w[t][c] = pow(1.0 + (net > 0 ? net : 0), -CBW);
                wtot += w[t][c];
            }
        }
...
int main(int argc, char **argv) {
    ...
    for (int tr = 1; tr <= MAX_TRIES; tr++) {
        init_random();
        long best = mono_total, since_improve = 0;
        for (long st = 0; st < MAX_STEPS; st++) {
            ...
            } else if (++since_improve >= PLATEAU) {                /* iterated local search: perturb ~2% of integers */
                int np = 1 + (int)(0.02 * N);
                for (int q = 0; q < np; q++) {
            ...
            if (getenv("DEBUG") && st % 100000 == 99999) {
                ...
                fprintf(stderr, "DEBUG try=%d step=%ld incr=%ld full=%ld %s\n",
                        tr, st, mono_total, fs, fs == mono_total ? "sync" : "DRIFT!");
                mono_total = fs; /* resync to keep the run meaningful */
            }
            step(st);
        }
        if (!getenv("QUIET"))
